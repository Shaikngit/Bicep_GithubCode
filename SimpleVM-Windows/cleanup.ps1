...
<#
.SYNOPSIS
    Cleanup script for Simple Windows VM deployment

.DESCRIPTION
    This script removes all resources created by the Simple Windows VM deployment.
    It provides options for complete cleanup or selective resource removal.

.PARAMETER ResourceGroupName
    Name of the resource group to clean up (default: rg-simple-windows-vm)
...
function Start-Cleanup {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-ColorOutput "ℹ️  Deletion is running in the background and may take 5-10 minutes to complete." "Yellow"
...
Write-ColorOutput "🧹 Simple Windows VM Cleanup Script" "Magenta"
